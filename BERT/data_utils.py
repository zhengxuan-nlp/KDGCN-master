...
def build_embedding_matrix(word2idx, embed_dim, dat_fname):
    if os.path.exists(dat_fname):
        ...
        fname = './glove.twitter.27B/glove.twitter.27B.' + str(embed_dim) + 'd.txt' \
            if embed_dim != 300 else './glove/glove.840B.300d.txt'  #'./glove.42B.300d.txt'
...
class ABSADataset(Dataset):
    def __init__(self, fname, tokenizer):
        ...
        fin = open(fname+'.concat_graph2', 'rb')
        idx2graph_bert = pickle.load(fin)
        fin.close()
        ...
        for i in range(0, len(lines), 3):
            ...
            # print('text:',text_left + " " + aspect + " " + text_right)


            dependency_graph_bert = np.pad(idx2graph_bert[i], \
                                      ((0, tokenizer.max_seq_len - idx2graph_bert[i].shape[0]),
                                       (0, tokenizer.max_seq_len - idx2graph_bert[i].shape[0])), 'constant')

            kg_feature = id2kg[i][0]
            kg_feature_bert = np.pad(id2kg[i][0],((0,0),(1,1)))
            ...
            if (len(aspect_expand) != 0):
                str = ' [SEP] '.join(id2aspect[i][1])+' [SEP]'
                aspect_expand_bert = tokenizer.text_to_sequence(str)
                aspect_expand = tokenizer.text_to_sequence(' '.join(id2aspect[i][1]))
            else:
                aspect_expand = tokenizer.text_to_sequence('.')
                aspect_expand_bert = tokenizer.text_to_sequence(' [SEP]')
            ...
            data = {
                ...
                'dependency_graph_bert': dependency_graph_bert,
                'polarity': polarity,
                'kg_feature': kg_feature,
                'kg_feature_bert': kg_feature_bert,
                'aspect_feature': aspect_feature,
                'aspect_expand': aspect_expand,
                'aspect_expand_bert': aspect_expand_bert,
            }
