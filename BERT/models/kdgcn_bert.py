...
class KDGCN_Bert(nn.Module):
    def __init__(self, bert, opt):
        super(KDGCN_Bert, self).__init__()
        self.opt = opt
        self.bert = bert
        self.dropout_bert = nn.Dropout(0.1)
        self.kg_gc = GraphConvolution(768+23, 768+23)
        self.prase_gc = GraphConvolution(768, 768)
        self.dropout_bert = nn.Dropout(0.1)


        self.dropout = nn.Dropout(opt.dropout)
        # self.pos_embed = nn.Embedding(opt.max_length, opt.position_dim)
        self.weight_n = nn.Parameter(torch.Tensor(768, 768))
        self.bias_n = nn.Parameter(torch.Tensor(1))
        self.attn = MultiHeadAttention(opt.num_head, 768+23)
        self.fc = nn.Linear(768*2+23, opt.polarities_dim)
    ...
    def forward(self, inputs):
        ...
        text_indices_bert,  aspect_indices_bert, left_indices_bert, parse_adj,  kg_feature, aspect_feature, aspect_expand = inputs

        text_len = torch.sum(text_indices_bert != 0, dim=-1)
        aspect_len = (torch.sum(aspect_indices_bert != 0, dim=-1))-2
        left_len = (torch.sum( left_indices_bert != 0, dim=-1)) - 1
        aspect_double_idx = torch.cat([left_len.unsqueeze(1), (left_len+aspect_len-1).unsqueeze(1)], dim=1)

        # bert
        x = self.bert(text_indices_bert)
        text_out = self.dropout_bert(x[0])

        x = self.bert(aspect_expand)
        aspect_expand_out = self.dropout_bert(x[0])

        # fuse knowleage_graph

        t_a_feature = torch.cat((text_out, aspect_expand_out), dim=1)  # text  aspect  feature

        fuse_feature = torch.zeros(t_a_feature.shape[0], t_a_feature.shape[1],
                                   t_a_feature.shape[2] + 23).cuda()  # 初始化矩阵
        ...
        score_mask = score_mask.unsqueeze(1).repeat(1, 1, 1, 1).cuda()
