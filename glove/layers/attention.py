...
class AttentionPosition(nn.Module):
    ...
    def forward(self, k, q):
        ...
        # score: (n_head*?, q_len, k_len,)
        # output: (?, q_len, out_dim,)
        # print("k.size(), q.size()", k.size(), q.size())
        kx = k.repeat(self.n_head, 1, 1).view(self.n_head, -1, self.embed_dim_k)  # (n_head, ?*k_len, embed_dim)
        qx = q.repeat(self.n_head, 1, 1).view(self.n_head, -1, self.embed_dim_q)  # (n_head, ?*q_len, embed_dim)
        # print("print(kx.size(), qx.size())", kx.size(), qx.size())
        kx = torch.bmm(kx, self.w_kx).view(-1, k_len, self.hidden_dim_k)  # (n_head*?, k_len, hidden_dim)
        qx = torch.bmm(qx, self.w_qx).view(-1, q_len, self.hidden_dim_q)  # (n_head*?, q_len, hidden_dim)
        # print("print(kx.size(), qx.size())", kx.size(), qx.size())
        kxx = kx
        qxx = qx
        kq = torch.cat((kxx, qxx), dim=-1)  # (n_head*?, k_len, hidden_dim*2)
        score = torch.tanh(torch.matmul(kq, self.weight))
        score = score.view(-1, k_len, 1) # (n_head*?, k_len, 1)
        score = F.softmax(score, dim=-1)
        output = kx * score
        ...
        output = torch.cat(torch.split(output, mb_size, dim=0), dim=-1)  # (?, n_head*hidden_dim)
        output = self.proj(output)  # (?, out_dim)
        output = self.dropout(output)
        return output, score