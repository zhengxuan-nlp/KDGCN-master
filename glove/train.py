...
class Instructor:
    def __init__(self, opt):
        ...
        self.word2idx = {v: k for k, v in absa_dataset.word2idx.items()}
    ...
    def idx2word(self,idx):
        ...
        for i in range(len(idx)) :
            idxs = idx[i]
            if idxs.item() != 0:
                words = words + ' ' + self.word2idx[idxs.item()]
        return words
    ...
    def _train(self, criterion, optimizer):
        ...
        for epoch in range(self.opt.num_epoch):
            ...
            for i_batch, sample_batched in enumerate(self.train_data_loader):
                ...
                inputs = [sample_batched[col].to(self.opt.device) for col in self.opt.inputs_cols]  #（N，L）
                targets = sample_batched['polarity'].to(self.opt.device)

                outputs = self.model(inputs)    #（N，3）
    ...
    def _evaluate_acc_f1(self):
        ...
        with torch.no_grad():
            for t_batch, t_sample_batched in enumerate(self.test_data_loader):
                ...
                if opt.load_model == 1:
                    sentence, aspect, _, _,_,_,_ = t_inputs
                    for i in range(len(t_sample_batched['text_indices'])):
                        sentences.append(self.idx2word(t_sample_batched['text_indices'][i]))
                    for i in range(len(t_sample_batched['aspect_indices'])):
                        aspects.append(self.idx2word(t_sample_batched['aspect_indices'][i]))
                    for i in t_targets:
                        polarity.append(i.item())
                    for i in torch.argmax(t_outputs, -1):
        ...
        if opt.load_model == 1 :
            ...
            polarity = map(transform, polarity)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', default='kdgcn', type=str)
    ...
    parser.add_argument('--num_head', default=1, type=int)      # (hidden_dim * 2 +23) % num_head  = 0
    ...
    parser.add_argument('--bert_model_dir', type=str, default='./bert_base')
    ...
    input_colses = {
        ...
        'kdgcn': ['text_indices', 'aspect_indices', 'left_indices', 'dependency_graph','kg_feature','aspect_feature','aspect_expand'],
    ...
    if opt.load_model == 0:
        ins.run()
    elif opt.load_model == 1:
        model = ins.model
        model.load_state_dict(torch.load('./state_dict/...'))
        test_acc, f1 = ins._evaluate_acc_f1()
        print('test_acc1: {:.4f}, f1: {:.4f}'.format(test_acc, f1))
